...
Data.addCourse = (title, location, startDate, endDate, attendeesMax, description, callback) => {
	connection.query("INSERT INTO courses (title, location, start_date, end_date, attendees_max, description, location_id, instructor_id) VALUES (?, ?, ?, ?, ?, ?, (SELECT id FROM sites WHERE name= ?),1)", ['%'.concat(title, '%'), location, startDate, endDate, attendeesMax, '%'.concat(description, '%'), location], function(err, rows, fields) {
		if (err) {
			return callback(err)
		}
		callback(null, { status: 200, responseJson: rows})
